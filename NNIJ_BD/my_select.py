...
def select_6():
    q6 = session.query(Student.stud_name, func.round(func.avg(Rating.grade), 2).label('avg_grade'))\
        .select_from(Rating).join(Student).group_by(Student.id).order_by(desc('avg_grade')).limit(5).all()
    return q6

# Знайти оцінки студентів у окремій групі з певного предмета.
def select_7():
    q7 = session.query(Student.stud_name, func.round(func.avg(Rating.grade), 2).label('avg_grade'))\
        .select_from(Rating).join(Student).group_by(Student.id).order_by(desc('avg_grade')).limit(5).all()
    return q7

# Знайти середній бал, який ставить певний викладач зі своїх предметів.
def select_8():
    q8 = session.query(Student.stud_name, func.round(func.avg(Rating.grade), 2).label('avg_grade'))\
        .select_from(Rating).join(Student).group_by(Student.id).order_by(desc('avg_grade')).limit(5).all()
    return q8

# Знайти список курсів, які відвідує певний студент.
def select_9():
    q9 = session.query(Student.stud_name, func.round(func.avg(Rating.grade), 2).label('avg_grade'))\
        .select_from(Rating).join(Student).group_by(Student.id).order_by(desc('avg_grade')).limit(5).all()
    return q9

# Список курсів, які певному студенту читає певний викладач.
def select_10():
    q10 = session.query(Student.stud_name, func.round(func.avg(Rating.grade), 2).label('avg_grade'))\
        .select_from(Rating).join(Student).group_by(Student.id).order_by(desc('avg_grade')).limit(5).all()
    return q10
